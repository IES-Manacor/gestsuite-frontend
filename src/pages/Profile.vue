<template>
  <q-page class="flex column" padding>
    ...
    <q-card flat bordered class="q-mb-md">
      ...
      <q-card-section>
        <div class="text-h6">Unitat Organitzativa</div>
      </q-card-section>
      <!--q-card-section class="q-pt-none">
        <q-input v-model="usuari.gsuiteUnitatOrganitzativa" label="Unitat Organitzativa" />
        <q-toggle v-model="usuari.bloquejaGsuiteUnitatOrganitzativa" label="Bloqueja unitat organitzativa"/>
        <p class="text-caption">Si bloqueja la unitat organitzativa, aquesta no s'actualitzarà a les pròximes sincronitzacions</p>
      </q-card-section-->
    </q-card>
  ...
  methods: {
    get: async function(){
      ...
      if(idusuari) {
        ...
        const responseGrupsCorreuUsuari = await this.$axios.get(process.env.API + '/api/core/grupcorreu/grupsusuari/' + idusuari);
        const dataGrupsCorreuUsuari = await responseGrupsCorreuUsuari.data;
        this.grupsCorreuUsuari = dataGrupsCorreuUsuari;
      }

      const responseGrupsCorreu = await this.$axios.get(process.env.API + '/api/core/grupcorreu/llistat')
      ...
      const responsePasswordInicial = await this.$axios.get(process.env.API + '/api/core/centre/password-inicial')
      this.newPassword = await responsePasswordInicial.data;
    },
    resetPassword: async function(){
      ...
      const responseResetPassword = await this.$axios.post(process.env.API + '/api/core/usuari/reset',{
    ...
    nouGrupUsuari: async function(){
      ...
      await this.$axios.post(process.env.API + '/api/core/grupcorreu/addmember', {
    ...
    esborrarUsuariGrup: async function(){
      ...
      await this.$axios.post(process.env.API + '/api/core/grupcorreu/removemember', {
